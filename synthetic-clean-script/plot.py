...
sb_type2_flag = "selection_bias_type2" in args.bias_type
...
for d in ds:
    ...
    for idx, (key, plot_name) in enumerate(target_vars.items()):
        ...
        if sb_type2_flag and "Y" not in key:
    ...
    for i,ax in enumerate(axs.flat):
        ...
        lower_bound = 0
        if sb_type2_flag and i != len(axs.flat)-1: 
            lower_bound = x_min
        ax.fill_between(
            x=[lower_bound, x_max],  # From x=0 to the current right limit
